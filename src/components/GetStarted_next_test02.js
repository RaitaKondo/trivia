...
export default function GetStarted(props) {
      ...
      if (round === 1) {
        ...
        for (let index = 0; index < hol.length; index++) {
          ...
          for (let index = 0; index < element.length; index++) {
            ...
            await Translator(el[0].value, props.formData.language).then(
              (val) => {
        ...
        for (let index = 0; index < CAhol.length; index++) {
          ...
          await Translator(element, props.formData.language).then((res) =>
        ...
        while (CAHolder.length > 0) {
          ...
          for (let i = 0; i < bro.length; i++) {
            const el = bro[i];
            await Translator(el, props.formData.language).then((res) =>
        ...
        while (holder.length > 0) {
          ...
          for (let index = 0; index < geo.length; index++) {
            ...
            await Translator(
              element[0].value,
              props.formData.language
            ).then((res) =>
              snake.push({ ...element[0], value: res.translatedText })
            );
            // todo:> question part --------------
            ...
            await Promise.all(
              element[1].map((i) =>
                Translator(i.value, props.formData.language)
                  .then((val) => todo.push({ ...i, value: val.translatedText }))
                  .catch((err) => console.log(err))
              )
            );
    ...
    async function getTriviaData() {
      try {
        ...
        for (let index = 0; index < resp.data.results.length; index++) {
          ...
          let combinedAnswers = await combineAllAnswers(
            element,
          ...
          let question = {
            value: removeCharacters(element.question),
            id: nanoid(),
          };

          // * make them a pair as an array. 上記の２つを一つのアレイにする事でペアにする。
          set.push([question, objectedAnswers]);
        }

        const ansBlock = [];

        // * make an array which stores the answer for each questions. それぞれのクイズの正解の答えだけをストックしているアレイを作る。
        const allCA = resp.data.results.map((result) =>
          removeCharacters(result.correct_answer)
        );
  ...
  const triviaItems = currentBlock.map((item) => {
    let c = item.map((i, index) => {
      ...
      question.push(<h1 key={i[0].id}>{i[0].value}</h1>);

      i[1].map((ans) => {
        const style =
          ans.correctAns && ans.isSelected
        ...
        anss.push(
          <button
            key={ans.id}
            onClick={(e) => changeSelected(e, ans.id, i[0].id)}
            className="btn"
            style={style}
          >
            {ans.value}
      ...
      return (
        ...
        <div
          key={cardKey}
          onMouseOver={() => (gs.dataset.activeIndex = index)}
          className="questionCard"
        >
          {question}
          <hr />
          <div className="answers">{anss}</div>
        </div>
      );
    });
    return c;
  ...
  const checkAns = () => {
    ...
    currentBlock.map((item) => {
      for (let index = 0; index < item.length; index++) {
        ...
        const test = element[1].map((ans) =>
          ans.value === correctAns
            ? { ...ans, correctAns: true }
            : { ...ans, correctAns: false }
        );
      ...
      {resultPage ? (
        ...
        <div className="main-container">
          ...
          {isLoading ? (
            ...
            <div>
              <div className="questions">{triviaItems}</div>
              <div id="menu-background-pattern"></div>
              <div id="menu-background-image"></div>

              <div className="bottom-buttons">
                {traLoading ? (
                  <h1 className="loading">Loading rests...</h1>
                ) : !answered ? (
                  <button className="btn" onClick={checkAns}>
                    Check Answers
                  </button>
                ) : (
                  ""
                ...
                {answered && done ? (
                  <button className="btn result" onClick={handleResult}>
                    Result Page
                  </button>
                ) : life > 0 && answered ? (
